...
import { type PointerEvent, type RefObject, useCallback } from 'react';
...
export const useSafeTriangle = (
  ...
  useCallback(
    (event: PointerEvent<HTMLElement>) => {
      ...
      style.setProperty(
        '--safe-left',
        `${(opensRight ? event.clientX : panelRect.right) - triggerRect.left}px`,
      );
      // 頂点=ポインタ位置（要素内 y 座標）、底辺=サブメニューの手前側の辺。
      style.setProperty('--safe-y', `${event.clientY - panelRect.top}px`);
      style.setProperty(
        '--safe-clip',
        opensRight
