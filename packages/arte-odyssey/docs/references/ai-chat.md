...
import { mapMessageParts } from '@k8o/arte-odyssey/ai-sdk';
```
...
使うサブパスの分だけインストールする。`@k8o/arte-odyssey/ai` 本体はどちらも不要。

```bash
# @k8o/arte-odyssey/ai/response（Response）を使う場合
pnpm add streamdown
# @k8o/arte-odyssey/ai-sdk（mapMessageParts）を使う場合
pnpm add ai
```
...
```tsx
import { Conversation } from '@k8o/arte-odyssey/ai';
...
```tsx
import { PromptInput } from '@k8o/arte-odyssey/ai';
...
```tsx
import { Reasoning } from '@k8o/arte-odyssey/ai';
...
```tsx
import { Suggestion } from '@k8o/arte-odyssey/ai';
...
`ChatStatus` / `ToolState` 型は `@k8o/arte-odyssey/ai-sdk` からも re-export される。
